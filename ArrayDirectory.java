...
public class ArrayDirectory  implements Directory {
	...
	ArrayDirectory(String filename){         //constructor responsible for adding the filename and running the FileEntries method 
	...
	public void ChangeNumber(String surname, int connection) {        //responsible for changing connection for given surname 
		...
		for(int j=0;j<array.length;j++) {
			if(surname.compareTo(array[j].getSurname()) != 0) {
				count++;
			}
		}
	...
	@Override
	public void LookUp(String surname) {    //responsible for returning information about given surname
		int count=0; //counts the number of times parameter surname didn't match with surname from array 
