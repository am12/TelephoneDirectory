...
public class ListDirectory implements Directory {
	...
	ListDirectory(String filename){    //constructor that inputs the filename and runs the fileEntries method 
	...
	@Override
	public void LookUp(String surname) {     //responsible for printing a given entry 
		...
		for(int i=0; i<array.size();i++) {
			if(surname.compareToIgnoreCase(array.get(i).getSurname())==0){  //if surname matches, the surname, initial and connection at that specific index will be printed 
				System.out.println(array.get(i).getSurname()+"\t"+array.get(i).getInitial()+"\t"+array.get(i).getConnection());
			}else {
	...
	public void ChangeNumber(String surname, int connection) {  //responsible for changing a connection for given surname
		...
		for(int j=0;j<array.size();j++) {
			if(surname.compareToIgnoreCase(array.get(j).getSurname())!=0) {
				count++;
			}
		}
		if(count==array.size()) {System.out.println("Error:Entry was not found");}
		
		  //counts the number of times parameter surname didn't match with surname from array 
		if(String.valueOf(connection).length()==5) {
		  ...
		  //if count is the same as array size,means the surname was not found anywhere in lineked list 
